...
function renderPage(source, release) {
  ...
  const platformStatusPt = macAvailable ? 'macOS preview disponível' : 'macOS em preparação';
  const platformStatusEn = macAvailable ? 'macOS preview available' : 'macOS coming soon';
  const faqPt = macAvailable
    ? `O instalador público está disponível para Windows x64. Um preview público para macOS Apple Silicon está disponível na <a href="${release.macReleaseUrl}" target="_blank" rel="noopener">página de versões do GitHub</a>. Ele é assinado apenas com a identidade local de desenvolvimento &quot;Matraca Dev&quot;; não tem certificado oficial Apple nem notarização. Não há versão Linux anunciada.`
  ...
  const faqEn = macAvailable
      ? `A public installer is available for Windows x64. A public preview for Apple Silicon macOS is available on the <a href="${release.macReleaseUrl}" target="_blank" rel="noopener">GitHub releases page</a>. It is signed only with the local development identity &quot;Matraca Dev&quot;; it has no Apple distribution certificate or notarization. No Linux release has been announced.`
  ...
  let output = source
    ...
    .replace(/Windows · macOS em preparação/g, `Windows · ${platformStatusPt}`)
    .replace(/Windows · macOS coming soon/g, `Windows · ${platformStatusEn}`)
  ...
  return output
    ...
    .replaceAll('__PLATFORM_STATUS_PT__', macAvailable ? 'macOS preview disponível' : 'macOS em preparação')
    .replaceAll('__PLATFORM_STATUS_EN__', macAvailable ? 'macOS preview available' : 'macOS coming soon')
